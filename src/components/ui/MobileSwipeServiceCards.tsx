...
export type MobileSwipeServiceCardItem = {
  id: string;
  title: string;
  description: string;
  href: string;
          ...
          <AnimatePresence initial={false} custom={direction} mode="popLayout">
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h3 className="mt-5 max-w-[12ch] text-balance text-[2rem] font-semibold tracking-[-0.04em] leading-[1.02] text-foreground">
                  {activeItem.title}
                </h3>
                ...
                {variant === 'rich' && activeItem.includes?.length ? (
                  <ul className="mt-6 grid gap-2">
                    {activeItem.includes.slice(0, 4).map((include) => (
                      <li
                        key={include}
                ...
                {variant === 'rich' && activeItem.expectedResultTitle ? (
                  <div className="mt-4 rounded-[1.35rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-4">
                    {expectedResultLabel ? (
                      <p className="text-[0.68rem] font-medium uppercase tracking-[0.16em] text-foreground/50">
                        {expectedResultLabel}
                      </p>
                    ...
                    <p className="mt-3 text-base font-medium leading-7 text-foreground">
                      {activeItem.expectedResultTitle}
                    </p>

                    {activeItem.expectedResultDescription ? (
                      <p className="mt-2 text-sm leading-7 text-muted-foreground">
                        {activeItem.expectedResultDescription}
                      </p>
                    ) : null}
                ...
                <div className="mt-6 border-t border-white/10 pt-5">
                  {variant === 'rich' && activeItem.detailsLabel ? (
                    <div className="mb-4">
                      <p className="text-xs uppercase tracking-[0.16em] text-foreground/45">
                        {activeItem.detailsLabel}
                      </p>
                      {activeItem.detailsDescription ? (
